...
const Main: React.FC<{ user: User | null; dark: boolean }> = ({
  user,
  dark,
}) => {
  const months = [
  ...
  const month = createdAt ? months[createdAt.getMonth()] : "";
  const year = createdAt ? createdAt.getFullYear() : "";
  const day = createdAt ? createdAt.getDate() : "";

  return (
    <div
      className={`pt-[3.2rem] px-[2.4rem] pb-[4.8rem] rounded-[1.5rem] ${
        dark ? "bg-[#1e2a47]" : "bg-[#fefefe] shadow-lg"
      } desktop:px-[4.8rem]`}
    >
      <div className="flex items-center gap-[2rem] mb-[3.4rem] desktop:gap-[3.7rem]">
        <img
          src={user?.avatar_url}
          alt="profile picture"
          className="w-[7rem] rounded-full md:w-[11.7rem] desktop:mt-[20px]"
        />
        <div className="desktop:flex desktop:justify-between desktop:w-[100%]">
          <div>
            <p
              className={`text-[1.6rem] font-bold ${
                dark ? "text-white" : "#2b3442"
              } md:text-[2.6rem]`}
            >
            ...
            <p
              className={`text-[1.5rem] mt-[20px] ${
                dark ? "text-[gray]" : "text-[#4b6a9b] "
              } mini:hidden desktop:block`}
            >
      ...
      {user?.bio && (
        <p
          className={`text-[1.3rem] mb-[2.2rem] ${
            dark ? "text-white" : "#4b6a9b"
          } md:text-[1.5rem] desktop:hidden`}
        >
          {user?.bio}
        </p>
      )}
      ...
      <div
        className={`flex justify-evenly text-center py-[1.9rem] 
          rounded-[10px] mb-[2.5rem] ${
            dark ? "bg-[#141d2f] text-white" : "bg-[#f6f8ff] text-[#2b3442]"
          } desktop:w-[77%] desktop:ml-auto monitor:w-[85%]`}
      >
        <div>
          <p
            className={`text-[1.1rem] mb-[8px] ${
              dark ? " text-white" : "text-[#4b6a9b]"
            } md:text-[1.3rem]`}
          >
            Repos
          </p>
          <p className="text-[1.6rem] font-bold md:text-[2.2rem]">
            {user?.public_repos}
          </p>
        </div>
      ...
      <div className="md:flex md:gap-[6.4rem] desktop:w-[77%] desktop:ml-auto monitor:w-[85%] monitor:gap-[30rem]">
        <div>
          <div
            className={`flex items-center gap-[1.9rem] mb-[1.6rem] ${
              dark ? "text-white" : "text-[#4b6a9b]"
            }`}
          >
            <svg
              ...
              fill={user?.location ? (dark ? "white" : "#4b6a9b") : "gray"}
            >
            ...
            {user?.location ? (
              <p className="text-[1.3rem]">{user?.location}</p>
            ) : (
              <p className="text-[1.3rem] text-[gray]">Not Available</p>
            )}
          </div>

          <div
            className={`flex items-center gap-[1.3rem] mb-[1.6rem] ${
              dark ? "text-white" : "text-[#4b6a9b]"
            } md:mb-0`}
          >
            <svg
              ...
              width="20"
              xmlns="http://www.w3.org/2000/svg"
              fill={user?.blog ? (dark ? "white" : "#4b6a9b") : "gray"}
            >
              <g>
        ...
        <div>
          ...
          >
            <svg
              ...
              xmlns="http://www.w3.org/2000/svg"
              fill={
                user?.twitter_username ? (dark ? "white" : "#4b6a9b") : "gray"
              }
            >
            ...
            {user?.twitter_username ? (
              <p className="text-[1.3rem]">{user?.twitter_username}</p>
            ) : (
              <p className="text-[1.3rem] text-[gray]">Not Available</p>
            )}
          </div>
          ...
          >
            <svg
              ...
              width="20"
              xmlns="http://www.w3.org/2000/svg"
              fill={user?.company ? (dark ? "white" : "#4b6a9b") : "gray"}
            >
              <g>
            ...
            {user?.company ? (
              <p className="text-[1.3rem]">{user.company}</p>
            ) : (
              <p className="text-[1.3rem] text-[gray]">Not Available</p>
            )}
          </div>
